...
describe('✅SwagLabs | Account | Iniciar sesión y BR de Accesos', () => {
    ...
    it('6517 | TC2: Validar login con credenciales correctas problem_user.', () => {
		cy.get("[type='text']")
            .type('problem_user')//user correcto
		cy.get("[type='password']")
            .type('secret_sauce')//password correcto
		cy.get('#login-button')
		cy.contains('Login')
            .click()
        cy.url("https://www.saucedemo.com/inventory.html").should("include", "inventory")
    ...
    it('6517 | TC3: Validar login con credenciales correctas performance_glitch_user', () => {
		cy.get("[type='text']")
            .type('performance_glitch_user')//user correcto
		cy.get("[type='password']")
            .type('secret_sauce')//password correcto
		cy.get('#login-button')
		cy.contains('Login')
	...
	it('6517 | TC4: Validar usuario no inicia sesion con cuenta bloqueada locked_out_user', () => {
		cy.get("[type='text']")
            .type('locked_out_user')//user incorrecta
		cy.get("[type='password']")
            .type('secret_sauce')//password incorrecta
		cy.get('#login-button')
		cy.contains('Login')
            .click()
    ...
    {
        cy.get("[type='text']")
            .type('standard_user')//user correcto
		cy.get("[type='password']")
            .type('asd12345')//password incorrecta
		cy.get('#login-button')
		cy.contains('Login')
            .click()
		cy.get("[data-test='error']")
	...
	it('6517 | TC6: Validar usuario inicia sesion con usuario incorrecto y password correcta.', () => {
        cy.get("[type='text']")
            .type('usuario_incorrecto')//user incorrecto
        cy.get("[type='password']")
            .type('secret_sauce')//password correcto
        cy.get('#login-button')
        cy.contains('Login')
	...
	it('6517 | TC7: Validar usuario inicia sesion con usuario valido y password null.', () => 
    {
        ...
        cy.get("[type='password']")
            .clear()
            .should('have.value', '')//password null
        cy.get('#login-button')
        cy.contains('Login')
            .click()
        cy.get("[data-test='error']")
